...
When(/^I click header's search (button|link)$/) do |target|
  @page.header.search_button.click if target == 'button'
  @page.header.search_link.click if target == 'link'
end
...
Then(/^I should (not see|see) header's search link$/) do |target|
  assert_equal(true, @page.header.search_link.displayed?) if target == 'see'
  assert_equal(false, @page.header.search_link.displayed?) if target == 'not see'
end
